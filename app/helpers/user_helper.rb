  ...
  def profile_btn(user)
    ...
    if !current_user.friend?(user)
      if current_user.pending_friends.include?(user)
        ...
        output << "#{link_to 'Accept', user_friendship_path(current_user, current_user.pending_friendship_ids[0]),
          class: 'btn btn-outline-primary', method: :patch}
          #{link_to 'Deny', user_friendships_path(user), class: 'btn btn-outline-secondary'}"
      else
        output << "<p class='px-2'>
       #{link_to 'Invite to Friend', user_friendships_path(user, current_user),
